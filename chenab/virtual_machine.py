...
class VirtualMachine(object):
    ...
    def byte_STORE_ATTR(self, name):
        val, obj = self.frame.pop_n(2)
        setattr(obj, name, val)

    def byte_STORE_SUBSCR(self):
        val, obj, subscr = self.frame.pop_n(3)
        obj[subscr] = val

    ## Building

    def byte_BUILD_TUPLE(self, count):
        elts = self.frame.pop_n(count)
    ...
    def byte_STORE_MAP(self):
        the_map, val, key = self.frame.pop_n(3)
        the_map[key] = val
    ...
    def byte_MAKE_FUNCTION(self, argc):
        name = self.frame.pop()
        code = self.frame.pop()
        defaults = self.frame.pop_n(argc)
    ...
    def byte_CALL_FUNCTION(self, arg):
        ...
        posargs = self.frame.pop_n(lenPos)
    ...
    def byte_IMPORT_NAME(self, name):
        level, fromlist = self.frame.pop_n(2)
        frame = self.frame
